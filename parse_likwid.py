...
def get_output(output_file, stat_str):
	...
	while line:
		...
		if stat_str in tokens:
			if stat_str == 'Runtime (RDTSC) [s] STAT':
...
def parse(input_file1,input_file2,conf,output_file):
	...
	parameters = {'Time':'Runtime (RDTSC) [s] STAT',
					'VPU_INSTRUCTIONS_EXECUTED':'VPU_INSTRUCTIONS_EXECUTED STAT',
					'VPU_ELEMENTS_ACTIVE':'VPU_ELEMENTS_ACTIVE STAT',
					'INSTRUCTIONS_EXECUTED':'INSTRUCTIONS_EXECUTED STAT',
					'CPU_CLK_UNHALTED':'CPU_CLK_UNHALTED STAT',
					'L2_DATA_READ_MISS_MEM_FILL':'L2_DATA_READ_MISS_MEM_FILL STAT',
					'DATA_CACHE_LINES_WRITTEN_BACK':'DATA_CACHE_LINES_WRITTEN_BACK STAT',
					'L2_DATA_WRITE_MISS_MEM_FILL':'L2_DATA_WRITE_MISS_MEM_FILL STAT'} 
	metrics = {}
	...
	BW = (metrics['L2_DATA_WRITE_MISS_MEM_FILL'] + metrics['L2_DATA_READ_MISS_MEM_FILL'] + 
		metrics['DATA_CACHE_LINES_WRITTEN_BACK']) * 64 * 1e-9 * int(conf[6])*int(conf[7])
	Vector = metrics['VPU_ELEMENTS_ACTIVE'] / metrics['VPU_INSTRUCTIONS_EXECUTED'] 
	FLOPS = metrics['VPU_ELEMENTS_ACTIVE'] * 1e-9 * int(conf[6])*int(conf[7])
