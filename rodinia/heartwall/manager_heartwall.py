...
def compiler():
    #compile program
    cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"main.c -I./AVI -c"
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)

    cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"main.o ./AVI/avimod.o ./AVI/avilib.o -o "+kernel_exec+" -lm"
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
    ...
    for affinity in ['scatter','balanced']:
        for cores in [19,38,57]:
            for threads in [2,3,4]:
                ...
                kernel_args = args+" "+str(cores*threads)
...
kernel_exec = "heartwall"
args = "test.avi 30"
CC = "icc"

out_file = "results/heartwall_30.csv"
