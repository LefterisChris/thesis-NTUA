...
def compiler():
	#compile program
	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"imagenet.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)

	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"backprop_kernel.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)

	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"backprop.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)

	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"facetrain.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)

	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"backprop.o facetrain.o imagenet.o backprop_kernel.o -o "+kernel_exec+" -lm"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)
...
def performer():
	...
	cmd = "scp test/"+kernel_exec+" mic0:/home/echristof"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
...
kernel_exec = "backprop"
kernel_args = "65536"
CC = "icc"

out_file = "results/backprop_65536.csv"
