...
def compiler():
	#compile program
	cmd = CC+" "+compilation_flags+"imagenet.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)

	cmd = CC+" "+compilation_flags+"backprop_kernel.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)

	cmd = CC+" "+compilation_flags+"backprop.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)

	cmd = CC+" "+compilation_flags+"facetrain.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)

	cmd = CC+" "+compilation_flags+"backprop.o facetrain.o imagenet.o backprop_kernel.o -o "+kernel_exec+" -lm"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)
...
out_file = "results/base_backprop_65536.csv"
