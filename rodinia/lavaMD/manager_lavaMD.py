...
def compiler():
	#compile program
	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"main.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)
	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"./kernel/kernel_cpu.c -c -o ./kernel/kernel_cpu.o"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)
	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"./util/num/num.c -c -o ./util/num/num.o"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)
	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"./util/timer/timer.c -c -o ./util/timer/timer.o"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)
	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"main.o ./kernel/kernel_cpu.o ./util/num/num.o ./util/timer/timer.o -o "+kernel_exec+" -lm"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)
...
kernel_exec = "lavaMD"
args = ["-cores","","-boxes1d","20"]
CC = "icc"

out_file = "results/lavaMD.csv"
