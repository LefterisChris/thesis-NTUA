...
def compiler():
    #compile program
    cmd = CC+" "+compilation_flags+"cluster.cpp -c"
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
    cmd = CC+" "+compilation_flags+"getopt.cpp -c"
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
    cmd = CC+" "+compilation_flags+"kmeans.cpp -c"
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
    cmd = CC+" "+compilation_flags+"kmeans_clustering.cpp -c"
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
    cmd = CC+" "+compilation_flags+"cluster.o getopt.o kmeans.o kmeans_clustering.o -o "+kernel_exec
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
...
out_file = "results/base_kmeans.csv"
