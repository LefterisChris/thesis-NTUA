...
def compiler():
    #compile program
    cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"cluster.cpp -c"
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
    cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"getopt.cpp -c"
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
    cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"kmeans.cpp -c"
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
    cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"kmeans_clustering.cpp -c"
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
    cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"cluster.o getopt.o kmeans.o kmeans_clustering.o -o "+kernel_exec
    ret = subprocess.call(cmd,shell=True)
    if(ret != 0):
        print "Error compile"
        sys.exit(1)
