...
def compiler():
	#compile program
	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"-I./common/ lud.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)
	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"-I./common/ lud_omp.c -c"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)
	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+"lud.o lud_omp.o ./common/common.o -o "+kernel_exec+" -lm"
	ret = subprocess.call(cmd,shell=True)
	if(ret != 0):
		print "Error compile"
		sys.exit(1)
