...
def performer():
	#compile
	cmd = CC+" "+compilation_flags+f1+f2+f3+f4+f5+path+"/"+kernel+" -o test/"+kernel_exec
	...
	for affinity in ['scatter','balanced']:
		for cores in [19,38,57]:
			for threads in [2,3,4]:
				...
				cmd = "cat out1.csv out2.csv out3.csv out4.csv > "+dst+"/out.csv"
				if subprocess.call(cmd,shell=True) != 0:
					print "Error"
					sys.exit(1)

				parse_likwid.parse(dst+"/out.csv","pwr.csv",conf,dst+out_file)

				cmd = "rm pwr.csv "+dst+"/out*"
				if subprocess.call(cmd,shell=True) != 0:
					print "Error"
					sys.exit(1)
